...
Func MemberPlacement($PlacementDistance = $GWEN_SCATTER_RANGE)
	If GetMapLoading() <> $instancetype_explorable Then Return
	Local $Angle = 0, $theta = 0, $TempX = 0, $TempY = 0
	Local $d2r = 3.141592653589 / 180
	Local $BotAngle = 17-(GUICtrlRead($Radio_Placement)) ;changed when west east orientation went mirrored This mirrors west/east but keeps nort&south same
	Local $aPartyLeader = $LeaderID
	Local $aPartyLeaderOrientation = Round(GetAgentRotationAngle_Ptr($aPartyLeader), 1) * 10
	Local $MyPlayerNumber = MemoryRead(GetAgentPtr(-2) + 244, "word")
	Local $PositiveAngle = (6 * $aPartyLeaderOrientation) + ((360 / 7) * $MyPlayerNumber)
	Local $NegativeAngle = (-6 * $aPartyLeaderOrientation) - ((360 / 7) * $MyPlayerNumber)
	Local $SetPositiveAngle = (6 * $aPartyLeaderOrientation) + ((360 / 16) * $BotAngle)
	Local $SetNegativeAngle = (-6 * $aPartyLeaderOrientation) - ((360 / 16) * $BotAngle)

	If $BotAngle = 0 Then
		If $aPartyLeaderOrientation >= 0 Then
			$theta = $PositiveAngle
			$TempX = ($PlacementDistance * Cos($theta * $d2r)) + X($aPartyLeader)
			$TempY = ($PlacementDistance * Sin($theta * $d2r)) + Y($aPartyLeader)
		ElseIf $aPartyLeaderOrientation < 0 Then
			$Angle = ($NegativeAngle * $aPartyLeaderOrientation) / $aPartyLeaderOrientation
			$theta = (360 - $Angle)
			$TempX = ($PlacementDistance * Cos($theta * $d2r)) + X($aPartyLeader)
			$TempY = ($PlacementDistance * Sin($theta * $d2r)) + Y($aPartyLeader)
		EndIf
	ElseIf $BotAngle <> 0 Then
		If $aPartyLeaderOrientation >= 0 Then
			$theta = $SetPositiveAngle
			$TempX = ($PlacementDistance * Cos($theta * $d2r)) + X($aPartyLeader)
			$TempY = ($PlacementDistance * Sin($theta * $d2r)) + Y($aPartyLeader)
		ElseIf $aPartyLeaderOrientation < 0 Then
			$Angle = ($SetNegativeAngle * $aPartyLeaderOrientation) / $aPartyLeaderOrientation
			$theta = (360 - $Angle)
			$TempX = ($PlacementDistance * Cos($theta * $d2r)) + X($aPartyLeader)
			$TempY = ($PlacementDistance * Sin($theta * $d2r)) + Y($aPartyLeader)
		EndIf
...
Func _Gwen_SaveLeaderPos()
	$g_i_Gwen_PrevLeaderX = $SavedLeaderX
	$g_i_Gwen_PrevLeaderY = $SavedLeaderY
EndFunc

Func _Gwen_ShouldReposition()
	If $LeaderID = "" Or $LeaderID = 0 Then Return False

	Local $lDist = GetDistance($LeaderID)
	If $lDist > $GWEN_SCATTER_RANGE + $GWEN_FOLLOW_CATCHUP Then Return True
	If $g_i_Gwen_PrevLeaderX = 0 And $g_i_Gwen_PrevLeaderY = 0 Then Return True
	Return _Gwen_LeaderMovedSinceLastPos($GWEN_FOLLOW_LEADER_THRESH)
EndFunc

Func _Gwen_RepositionFollower()
	If GetMapLoading() <> $instancetype_explorable Then Return False

	Local $lMoved = False
	If GetDistance($LeaderID) > $GWEN_SCATTER_RANGE + $GWEN_FOLLOW_CATCHUP _
		Or _Gwen_LeaderMovedSinceLastPos($GWEN_FOLLOW_STEP_THRESH) Then
		; Always step toward the leader — never hold a scatter ring (that made her run away)
		$lMoved = Move($SavedLeaderX, $SavedLeaderY, 15)
	EndIf

	If $lMoved Then _Gwen_SaveLeaderPos()
	Return $lMoved
EndFunc

Func _Gwen_LeaderMovedSinceLastPos($a_f_Threshold = 40)
	If $g_i_Gwen_PrevLeaderX = 0 And $g_i_Gwen_PrevLeaderY = 0 Then Return True
	Return ComputeDistance($SavedLeaderX, $SavedLeaderY, $g_i_Gwen_PrevLeaderX, $g_i_Gwen_PrevLeaderY) > $a_f_Threshold
EndFunc

Func _Gwen_GetTargetID($aTarget)
	If IsDllStruct($aTarget) Then Return DllStructGetData($aTarget, "ID")
	If IsPtr($aTarget) Then Return Agent_GetAgentInfo($aTarget, "ID")
	If IsNumber($aTarget) And $aTarget <> 0 Then Return $aTarget
	Return 0
EndFunc

Func _Gwen_MaxFightDistFromLeader()
	Return $GWEN_SCATTER_RANGE + $GWEN_AGGRO_RANGE
EndFunc

#Region Gwen Follow & Combat
Func Move($a_f_X, $a_f_Y, $a_f_Random = 0)
	If Map_GetInstanceInfo("IsOutpost") Then Return False
	If Map_GetInstanceInfo("IsLoading") Then Return False

	Local $l_i_Throttle = $GWEN_FOLLOW_MOVE_MS
	If $LeaderID <> "" And $LeaderID <> 0 Then
		If GetDistance($LeaderID) > $GWEN_SCATTER_RANGE + $GWEN_FOLLOW_CATCHUP Then
			$l_i_Throttle = $GWEN_FOLLOW_CATCHUP_MOVE_MS
		EndIf
	EndIf
	If $g_i_Gwen_LastMoveTime <> 0 And TimerDiff($g_i_Gwen_LastMoveTime) < $l_i_Throttle Then Return False
	$g_i_Gwen_LastMoveTime = TimerInit()
	Return Map_Move($a_f_X, $a_f_Y, $a_f_Random)
EndFunc

Func ChangeTarget($aTarget)
	If IsDllStruct($aTarget) Then Return Agent_ChangeTarget(DllStructGetData($aTarget, "ID"))
	Return Agent_ChangeTarget($aTarget)
EndFunc

Func TargetNearestEnemy()
	Return Agent_TargetNearestEnemy()
EndFunc

Func TargetCalledTarget()
	Local $l_i_Target = Party_GetMyPartyPlayerMemberInfo(1, "CalledTargetID")
	If $l_i_Target <> 0 Then Agent_ChangeTarget($l_i_Target)
EndFunc

Func _Gwen_PickFightTarget()
	Local $l_i_Target = _Gwen_GetTargetID(GetCurrentTarget())
	If $l_i_Target <> 0 _
		And Agent_GetAgentInfo($l_i_Target, "Allegiance") = 3 _
		And Agent_GetAgentInfo($l_i_Target, "HP") > 0.005 _
		And GetDistance($l_i_Target, -2) <= $GWEN_AGGRO_RANGE _
		And GetDistance($l_i_Target, $LeaderID) <= _Gwen_MaxFightDistFromLeader() Then
		Return $l_i_Target
	EndIf

	$l_i_Target = _Gwen_GetTargetID(GetNearestEnemyToAgent(-2, $GWEN_AGGRO_RANGE))
	If $l_i_Target = 0 Then Return 0
	If Agent_GetAgentInfo($l_i_Target, "Allegiance") <> 3 Then Return 0
	If Agent_GetAgentInfo($l_i_Target, "HP") <= 0.005 Then Return 0
	If GetDistance($l_i_Target, -2) > $GWEN_AGGRO_RANGE Then Return 0
	If GetDistance($l_i_Target, $LeaderID) > _Gwen_MaxFightDistFromLeader() Then Return 0
	Return $l_i_Target
EndFunc

Func _Gwen_TickFight($a_i_TargetID)
	If $a_i_TargetID = 0 Then Return False
	ChangeTarget($a_i_TargetID)
	Agent_Attack($a_i_TargetID)

	For $l_i_Slot = 1 To 8
		If GUICtrlRead($CheckboxSkill[$l_i_Slot]) <> $GUI_CHECKED Then ContinueLoop
		If GetEffectTimeRemaining($SkillID[$l_i_Slot]) >= 1500 Then ContinueLoop
		If GetSkillbarSkillAdrenaline($l_i_Slot, 0) < $AdrenalineSkill[$l_i_Slot] Then ContinueLoop
		UseSkillEx($l_i_Slot, $a_i_TargetID)
		ExitLoop
	Next
	Return True
EndFunc

Func _Gwen_FollowAndFight()
	If GetIsDead(-2) Then Return -1
	If GetMapLoading() <> $instancetype_explorable Then Return False

	Local $l_b_Fight = (GUICtrlRead($FightYes) == $GUI_CHECKED)
	Local $l_i_LeaderDist = GetDistance($LeaderID)
	Local $l_i_FightTarget = 0

	If $l_b_Fight Then
		TargetCalledTarget()
		$l_i_FightTarget = _Gwen_PickFightTarget()
		If $l_i_FightTarget <> 0 Then
			_Gwen_TickFight($l_i_FightTarget)
			; Keep fighting — don't issue follow moves that cancel attacks
			If $l_i_LeaderDist <= $GWEN_SCATTER_RANGE + $GWEN_FOLLOW_CATCHUP Then Return True
		EndIf
	EndIf

	If _Gwen_ShouldReposition() Then _Gwen_RepositionFollower()
	Return True
EndFunc

Func MoveAndKillFCT($lX, $lY, $LeaderID, $lS = "")
	Return _Gwen_FollowAndFight()
EndFunc

Func FightFCT()
	Local $l_i_Target = _Gwen_PickFightTarget()
	If $l_i_Target = 0 Then Return False
	Return _Gwen_TickFight($l_i_Target)
EndFunc
#EndRegion
...
If Not $Partyleader Then
		CheckCurrentPartyleader()
	EndIf

	If $LeaderID = "" Or $SavedMap <> GetMapID() Or DllStructGetData(GetAgentByID($LeaderID), "X") = 0 Then
		GetPartyLeader()
		If $LeaderID = "" Or DllStructGetData(GetAgentByID($LeaderID), "X") = 0 Then
			RndSleep(250)
			Return
		EndIf
	EndIf

	$SavedLeaderX = Round(DllStructGetData(GetAgentByID($LeaderID), "X"))
	$SavedLeaderY = Round(DllStructGetData(GetAgentByID($LeaderID), "Y"))

	Static $g_i_Gwen_LastMap = 0
	If GetMapID() <> $g_i_Gwen_LastMap Then
		$g_i_Gwen_PrevLeaderX = 0
		$g_i_Gwen_PrevLeaderY = 0
		$g_i_Gwen_LastMoveTime = 0
		$g_i_Gwen_LastLootScan = 0
		$g_i_Gwen_LastPconCheck = 0
		$g_i_Gwen_LastMap = GetMapID()
	EndIf

	If Not Map_GetInstanceInfo("IsExplorable") Then Return

	If GetIsDead(-2) Then Return -1
	_Gwen_FollowAndFight()

	If GUICtrlRead($FPickup) == $GUI_CHECKED Then
		If GUICtrlRead($OpenChests) == $GUI_CHECKED Then
			CheckForChest()
		EndIf

		FollowBot_PickUpLoot()
	EndIf
EndFunc ;==> Main

Func _Gwen_InitMapResignTracking()
	$MapResignID = GetMapID()
	$g_b_Gwen_SkipNextResign = Map_GetInstanceInfo("IsExplorable")
	$g_b_Gwen_ResignedThisZone = $g_b_Gwen_SkipNextResign
EndFunc

Func _Gwen_TryResignOnZoneEntry()
	If $g_b_Gwen_ResignedThisZone Then Return
	If Not Map_GetInstanceInfo("IsExplorable") Then Return

	If $g_b_Gwen_SkipNextResign Then
		$g_b_Gwen_SkipNextResign = False
		$g_b_Gwen_ResignedThisZone = True
		$MapResignID = GetMapID()
		Return
	EndIf
	...
	$g_b_Gwen_ResignedThisZone = True
	$MapResignID = $l_i_Map
	Out("Entering map " & $l_i_Map & " - resigning to sync with party.")

	; Do not block follow on zone entry. The old Map_WaitMapLoading(15000)
	; used 15000 as a map id, and Other_PingSleep(500) delayed the first follow move.
	Other_PingSleep(100)
	Resign()
EndFunc
...
Func UsePCons()
	For $l_i_Idx = 0 To 8
		If GetEffectTimeRemaining($g_a_Gwen_KnownPcons[$l_i_Idx][1]) = 0 Then
			Local $l_i_Item = GetItemInInventory($g_a_Gwen_KnownPcons[$l_i_Idx][0])
			If $l_i_Item <> 0 Then UseItem($l_i_Item)
		EndIf
	Next

	; Any other pcons in inventory (no GUI selection)
	Local $l_ap_Items = Item_GetItemArray()
	If Not IsArray($l_ap_Items) Then Return

	Local $l_a_UsedModels[1] = [0]
	For $l_i_Idx = 0 To 8
		_Gwen_AddUsedModel($l_a_UsedModels, $g_a_Gwen_KnownPcons[$l_i_Idx][0])
	Next

	For $l_i_Idx = 1 To $l_ap_Items[0]
		Local $l_p_Item = $l_ap_Items[$l_i_Idx]
		Local $l_i_Model = Memory_Read($l_p_Item + 0x2C, "dword")
		If _Gwen_ModelAlreadyUsed($l_a_UsedModels, $l_i_Model) Then ContinueLoop
		If Not CheckArrayPscon($l_i_Model) And Not IsPcon($l_p_Item) Then ContinueLoop

		Local $l_i_ItemID = Memory_Read($l_p_Item, "dword")
		If $l_i_ItemID <> 0 Then
			UseItem($l_i_ItemID)
			_Gwen_AddUsedModel($l_a_UsedModels, $l_i_Model)
		EndIf
	Next
...
Func _Gwen_AddUsedModel(ByRef $a_a_Models, $a_i_Model)
	For $l_i_Idx = 1 To $a_a_Models[0]
		If $a_a_Models[$l_i_Idx] = $a_i_Model Then Return
	Next
	$a_a_Models[0] += 1
	ReDim $a_a_Models[$a_a_Models[0] + 1]
	$a_a_Models[$a_a_Models[0]] = $a_i_Model
EndFunc

Func _Gwen_ModelAlreadyUsed(ByRef $a_a_Models, $a_i_Model)
	For $l_i_Idx = 1 To $a_a_Models[0]
		If $a_a_Models[$l_i_Idx] = $a_i_Model Then Return True
	Next
	Return False
EndFunc

Func FollowBot_PickUpLoot()
	If $g_i_Gwen_LastLootScan <> 0 And TimerDiff($g_i_Gwen_LastLootScan) < 750 Then Return True
	$g_i_Gwen_LastLootScan = TimerInit()

	Local $lDistanceToAggro = 0
	Local $lMe
	Local $lBlockedTimer
	Local $lBlockedCount = 0
	Local $lItemExists = True
	Local $Distance
	;Out("Check if there's loot lying around")
	For $i = 1 To GetMaxAgents()
		local $lAgent = GetAgentByID($i)
		If Not GetIsMovable($lAgent) Then ContinueLoop
		local $lDistance = GetDistance($lAgent)
	    	If $lDistance > 1200 Then ContinueLoop
		local $lItem = GetItemByAgentID($i)
		Local $lModelID = DllStructGetData($lItem, 'ModelID')
		Local $lItemAgentID = DllStructGetData($lItem, 'AgentID')
		If $lModelID = 0 Or $lItemAgentID = 0 Then ContinueLoop

		;Can loot without drawing aggro?
		local $LootX = DllStructGetData($lAgent, 'X')
		local $LootY = DllStructGetData($lAgent, 'Y')
		local $LootID = DllStructGetData($lAgent, 'ID')
		If $LootID = 0 Then ContinueLoop
		Local $nearestenemy = GetNearestEnemyToAgent($LootID)
		If $nearestenemy = 0 Then
			Local $lDistanceToAggro = 9999
		Else
			Local $lDistanceToAggro = GetDistance($nearestenemy, $LootID)
		EndIf
		If $lDistanceToAggro < 1200 Then ContinueLoop ;check if coast is clear to pick up loot
		; Party flag: only when leader dropped it nearby (5899 ID kits drop as loot and break follow)
		If GetMapLoading() = $instancetype_explorable _
			And $lModelID = $GWEN_FLAG_MODEL_ID _
			And $LeaderID <> "" _
			And GetDistance($lAgent, GetAgentByID($LeaderID)) < 800 Then
			FlagAll($lModelID, $lAgent)
			ContinueLoop
		EndIf
		If $lModelID = $GWEN_FLAG_MODEL_ID Then ContinueLoop
		If FollowBot_CanPickUp($lItem, $lAgent) Then
			Out("Found loot I can pick up")
			Do
		...
		Out("All loot picked up")

		;If empty bagslots getting low, notify partyleader
			Local $EmptyBag = CountAllEmptySlots()
			If ($EmptyBag <= 5) Then
				Local $aMessage = "Only got "&$EmptyBag&" empty bagslots left!"
				SendWhisper($LeaderName, $aMessage)
			EndIf
		EndIf
	Next
	;Out("Lootcheck done")
Return True
EndFunc	;=> FollowBot_PickUpLoot

Func FollowBot_CheckArrayPscon($lModelID)
...
; when thrown on ground, all followers will be flagged
Func FlagAll($Modelid, $Agent2)
		Local $XFlag = DllStructGetData($Agent2, 'X')
		Local $YFlag = DllStructGetData($Agent2, 'Y')
		If GetDistance($Agent2) > 50 Then
			Move($XFlag, $YFlag)
			Sleep(2000)
			Out("Followers flagged!")
		EndIf
FollowBot_PickUpLoot()
EndFunc

Func FollowBot_CanPickUp($aitem, $lAgent)
	...
	Local $CurGold = GetGoldCharacter()
	...
	;Check if dropped item is assigned to me
	ElseIf GetAssignedToMe($lAgent) == True Then
		If ($type == $TYPE_KEY) Then ; dungeon key
		Return True
		ElseIf (CheckArrayMaterials($lModelID) And (GUICtrlRead($AllMats) = 1)) Then ; Pcons
		Return True
		ElseIf (CheckArrayRareMaterials($lModelID) And (GUICtrlRead($RareMats) = 1)) Then ; Pcons
		Return True
		ElseIf (FollowBot_CheckArrayPscon($lModelID) And (GUICtrlRead($PConsLoot) = 1)) Then ; Pcons
		Return True
		ElseIf (CheckArrayTomes($lModelID) And (GUICtrlRead($Tomes) = 1)) Then ; Tomes
		Return True
		ElseIf (($lRarity == $Rarity_Green) And (GUICtrlRead($FPickupGreen) = 1)) Then ; Green Items
		Return True
		ElseIf (($lRarity == $Rarity_Gold) And (GUICtrlRead($FPickupGold) = 1)) Then ; Gold Items
		Return True
		ElseIf ($lRarity == $Rarity_Purple) And (GUICtrlRead($FPickupPurple) = 1) Then ; Purple Items
		Return True
		ElseIf (($lRarity == $Rarity_Blue) And (GUICtrlRead($FPickupBlue) = 1)) Then ; Blue Items
		Return True
		ElseIf (($lRarity == $Rarity_White) And (GUICtrlRead($FPickupWhite) = 1)) Then ; White Items
		Return True
		ElseIf ($lModelID == $Item_ID_Dyes) AND ($aExtraID == $Item_ExtraID_BlackDye) Then
		Return True
		ElseIf ($lModelID == $Item_ID_Dyes) AND ($aExtraID == $Item_ExtraID_WhiteDye) Then
		Return True
		ElseIf ($lModelID == $Item_ID_Dyes) AND	($aExtraID == $Item_ExtraID_PinkDye) Then
		Return True
		ElseIf ($lModelID == $NicksItem) Then
		Return True
		EndIf
...
EndFunc	;=> CanPickUp
 ...
 Func CheckForChest()
	...
	;Local $NRLockpicks = GUICtrlRead($NRLockpicks)
	If $NRLockpicks == 0 Then Return
	...
	Local $aMessage = "CHEST IN SIGHT! I'M GONNA OPEN IT"
	SendWhisper($LeaderName, $aMessage)
	...
	GUICtrlSetData($OpenChestsLabel, "Open Chests(" & $NRLockpicks & ")")
...
Func AggroMoveSmartFilter($aX, $aY, $aRange = 1700, $aFilter = "EnemyFilter")
    AggroMoveToExFilter($aX, $aY, $aRange, $aFilter)
EndFunc
...
#Region Constants
; Map types: use $instancetype_* from Bot_Constants.au3 (same values as GwAu3 $GC_I_MAP_TYPE_*).
; Ranges: use $adjacent_range, $earshot_range, etc. from Bot_Constants.au3.
...
; Item rarity / lockpicks: see Bot_Constants.au3 ($Rarity_*, $Item_ID_Lockpicks)
...
; Material/tome arrays: see Bot_Constants.au3
